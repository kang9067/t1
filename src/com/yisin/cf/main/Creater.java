...
public class Creater {
	...
	public void run() {
		String result = createEntity();//OK
		log.info(result);
		result = createBaseArgument();
		log.info(result);
		// 持久层框架
		if(ca.getDaoFrame().equalsIgnoreCase("mybatis")){
		    result = createIDao();//ok
		    log.info(result);
		    result = createDao();//ok
		    log.info(result);
		    result = createDaoMapper();//ok
		    log.info(result);
		    result = createIBaseService();//--
		    log.info(result);
		    result = createBaseService();//--
		    log.info(result);
		    result = createIService();//ok
		    log.info(result);
		    result = createService();//ok
		    log.info(result);
		    result = createMapper();
		    log.info(result);
		} else {
	...
	public String createMapper() {
		...
		try {
			...
			// package
			content = Utililies.parseTemplate(content, "MapperPackage", ca.getMapPackage());//1
			content = Utililies.parseTemplate(content, "EntityPackage", ca.getEntityPackage());
		...
		if (columnList != null) {
			...
			if(DBHelper.sections.equals(MySqlSetPanel.sections)){
    			for (int i = 0, k = columnList.size(); i < k; i++) {
    				...
    				if (column != null 
    						//去除createTime modifyTime 和id
    						&& !column.getColumnName().toLowerCase().equals("modifytime")
    					...
    					sb.append(Consts.TAB1).append("/** ").append(CommonUtils.isBlank(column.getColumnComment()) ? column.getColumnName(): column.getColumnComment()).append(" */")//添加注释
	...
	public String createAttrGetsetList() {
		...
		if (columnList != null) {
			...
			if(DBHelper.sections.equals(MySqlSetPanel.sections)){
    			for (int i = 0, k = columnList.size(); i < k; i++) {
    				column = (MysqlDbColumn) columnList.get(i);
    				if (column != null //去除createTime modifyTime 和id
    						&& !column.getColumnName().toLowerCase().equals("modifytime")
    						&& !column.getColumnName().toLowerCase().equals("createtime")
    						&& !column.getColumnName().toLowerCase().equals("id")) {
	...
	public String getFeildMapList(){
	    ...
	    if(columnList != null){
            if(DBHelper.sections.equals(MySqlSetPanel.sections)){
                MysqlDbColumn mdc = null;
                String maps = "<result column=\"{0}\" property=\"{2}\" />";
                for (int i = 0, k = columnList.size(); i < k; i++) {
                    mdc = (MysqlDbColumn) columnList.get(i);
                    if(!mdc.isPrimaryKey()){
        ...
        if(columnList != null){
            if(DBHelper.sections.equals(MySqlSetPanel.sections)){
                ...
                feild.append("<if test=\"{0} != null\" >\r\n")
                    .append(Consts.TAB3).append("and {1} = #{{2}}\r\n")
